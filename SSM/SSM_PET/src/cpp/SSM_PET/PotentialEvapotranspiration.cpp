...
double PotentialEvapotranspiration::getalbedo() { return this->albedo; }
void PotentialEvapotranspiration::setalbedo(double _albedo) { this->albedo = _albedo; }
void PotentialEvapotranspiration::Calculate_Model(petState &s, petState &s1, petRate &r, petAuxiliary &a, petExogenous &ex)
{
    ...
    {
        pet = eeq * 0.01 * std::exp(0.18 * (tmax + 20.0));
    }
    s.setpet(pet);
}