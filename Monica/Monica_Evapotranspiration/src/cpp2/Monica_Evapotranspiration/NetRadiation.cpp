...
void NetRadiation::Calculate_Model(ETState &s, ETState &s1, ETRate &r, ETAuxiliary &a, ETExogenous &ex)
{
    ...
    clear_sky_solar_radiation = (0.75 + (0.00002 * height_nn)) * a.extraterrestrial_radiation;
    ...
    longwave_radiation = bolzmann_constant * ((std::pow(ex.min_air_temperature + 273.16, 4.0) + std::pow(ex.max_air_temperature + 273.16, 4.0)) / 2.0) * (1.35 * relative_shortwave_radiation - 0.35) * (0.34 - (0.14 * std::sqrt(a.vapor_pressure)));
