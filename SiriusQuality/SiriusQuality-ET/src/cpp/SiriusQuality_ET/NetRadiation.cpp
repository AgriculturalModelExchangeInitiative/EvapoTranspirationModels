...
void NetRadiation::setalbedoCoefficientCan(double _albedoCoefficientCan) { this->albedoCoefficientCan = _albedoCoefficientCan; }
void NetRadiation::Calculate_Model(EnergyBalanceCompositeState &s, EnergyBalanceCompositeState &s1, EnergyBalanceCompositeRate &r, EnergyBalanceCompositeAuxiliary &a, EnergyBalanceCompositeExogenous &ex)
{
    ...
    clearSkySolarRadiation = (0.750 + (2 * std::pow(10.00, -5) * elevation)) * extraSolarRadiation;
    averageT = (std::pow(maxTair + 273.160, 4) + std::pow(minTair + 273.160, 4)) / 2.00;
    surfaceEmissivity = 0.340 - (0.140 * std::sqrt(vaporPressure / 10.00));
