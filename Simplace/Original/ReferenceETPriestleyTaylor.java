...
public class ReferenceETPriestleyTaylor extends FWSimComponent
{
	...
	protected void process()
	{
		...
		double Delta = EquationsFAO56.SlopeOfSaturationVapPressureCurve(T); //slope of saturation vapour pressure curve [kPa °C-1] Allen et al. (1998) Eq[13]
		double AtmPres = EquationsFAO56.AtmosphericPressure(cAltitude.getValue()); // atmospheric pressure [kPa] Allen et al. (1998) Eq[7]
		double Gamma = EquationsFAO56.PsychrometricConstant(AtmPres); //psychrometric constant [kPa °C-1] Allen et al. (1998) Eq[8]
		double G = 0.0; // Soil heat flux (Allen et al, 1998) [W m-2] Eq[45] and Eq[46] 	
		
		ReferenceCropEvapotranspiration.setValue( max(0, cAlphaPT.getValue() * Delta/(Delta + Gamma) * (iNetRadiation.getValue() - G)/EquationsFAO56.lambda ), this );
	}
