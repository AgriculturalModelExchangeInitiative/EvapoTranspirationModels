from . import ReferenceETPM_Component
import pandas as pd
import os
...
def simulation(datafile, vardata, params, init):
    ...
    t_iActualVapourPressure = df[vardata.loc[vardata["Variables"]=="iActualVapourPressure","Data columns"].iloc[0]].to_list()
    ...
    t_iWindspeed = df[vardata.loc[vardata["Variables"]=="iWindspeed","Data columns"].iloc[0]].to_list()
    ...
    for i in range(0,len(df.index)-1):
        ...
        iActualVapourPressure = t_iActualVapourPressure[i]
        iTMax = t_iTMax[i]
        iTMin = t_iTMin[i]
        iWindspeed = t_iWindspeed[i]
        ReferenceCropEvapotranspiration= ReferenceETPM_Component.model_referenceetpm_(iNetRadiation,cAltitude,iActualVapourPressure,iTMax,iTMin,iWindspeed)
